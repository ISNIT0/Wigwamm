...
app.get('/app/features/:location/:beds', function(req, res) { //Selecting categories
  ...
  categoryMap = categoryMap.map(function(val) {
    val.count = JSON.parse(request('GET', 'http://api.nestoria.co.uk/api?action=search_listings&encoding=json&place_name=' + req.params.location +
      '&listing_type=buy&bedroom_min=' + req.params.beds +
      '&bedroom_max=' + req.params.beds +
      '&keywords=' + (getCategoryById(val.id) || {
        keys: []
      }).keys.join(',')).getBody()).response.listings.length
    return val;
  });
  ...
  load('views/appProperties.html').then(function(Component) {
    db.userRemoved.find({
      ...
      var Properties = JSON.parse(request('GET', 'http://api.nestoria.co.uk/api?action=search_listings&encoding=json&place_name=' + req.params.location +
        '&listing_type=buy&bedroom_min=' + req.params.beds +
        '&bedroom_max=' + req.params.beds +
        '&keywords=' + (getCategoryById(req.params.category) || {
          keys: []
        }).keys.join(',')).getBody()).response.listings;
